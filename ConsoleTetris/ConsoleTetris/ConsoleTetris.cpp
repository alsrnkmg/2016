...
void KBlock::SetBlockType(KBlock* pTargetBlock, char cBlockType){
	...
	{
		m_aBlockPosition[0].x = 0;	// @0#1
		m_aBlockPosition[0].y = 0;	// #2
		...
		;
	else if (cBlockType == 'd')
		;
	else if (cBlockType == 'e')
		;
	else if (cBlockType == 'f')
		;
	else if (cBlockType == 'g')
		;
	else
		;	// ERROR

	::memcpy(pTargetBlock, this->m_aBlockPosition, sizeof(m_aBlockPosition) * m_iNumStone);
}

KBlock      g_curBlock;
float       g_fCurBlockTimer = 0.f;
float       g_fCurBlockSpeed = 0.1f; ///< unit is seconds per a block
...
void OnUpdate(float fElapsedTime_) {
	...
	const bool bLocate = g_curBlock.CanLocateOnBoard();
	if (bLocate == false) {
